...
class DecorationElementContainerWidgetState
  ...
  bool downSelectTapOtherAction(PointerDownEvent event) {
    ...
    DecorationElement selectedDwg = mSelectedWg as DecorationElement;
    if (selectedDwg.isInScaleAndRotateButton(x, y)) {
      // 开始进行单指旋转缩放
      mDecorationActionMode =
          DecorationActionMode.SINGER_FINGER_SCALE_AND_ROTATE;
      selectedDwg.onSingleFingerScaleAndRotateStart();
      // 实测 这里啥也没做，
    ...
    if (selectedDwg.isInRemoveButton(x, y)) {
  ...
  bool scrollSelectTapOtherAction(List<DragUpdateDetails> dragUpdateDetails) {
    ...
    if (mDecorationActionMode ==
        DecorationActionMode.SINGER_FINGER_SCALE_AND_ROTATE) {
      DecorationElement selectedDecorationElement = mSelectedWg! as DecorationElement;
